...
import {
  ...
  GraduationCap, Hash, Crown, ChevronRight, DoorOpen, UserMinus
} from "lucide-react";
...
import ClassWordSets from "@/components/ClassWordSets";
import ClassGameLauncher from "@/components/ClassGameLauncher";
...
export default function Classroom() {
  ...
  const [adminStep, setAdminStep] = useState(true);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [copied, setCopied] = useState(false);
  const [members, setMembers] = useState<MemberWithProfile[]>([]);
  const [teacherProfile, setTeacherProfile] = useState<MemberWithProfile | null>(null);
  ...
  const fetchClasses = useCallback(async () => {
    ...
    if (memberships && memberships.length > 0) {
      ...
      const { data: joined } = await supabase
        .from("classes").select("*").in("id", classIds).neq("teacher_id", user.id);
    ...
    for (const cid of allClassIds) {
      const { count } = await supabase
        .from("class_members").select("*", { count: "exact", head: true }).eq("class_id", cid);
      counts[cid] = count || 0;
    }
  ...
  const verifyAdminCode = async () => {
    ...
    const { data } = await supabase
      .from("teacher_codes").select("*").eq("code", adminCode.trim().toUpperCase()).eq("used", false).single();
    if (!data) { setError("Invalid or already used teacher code."); setLoading(false); return; }
    await supabase.from("teacher_codes")
      .update({ used: true, used_by: user.id, used_at: new Date().toISOString() }).eq("id", data.id);
  ...
  const createClass = async () => {
    ...
    const { error: err } = await supabase.from("classes").insert({
      name: className.trim(), description: classDesc.trim(), code, teacher_id: user.id,
    });
    if (err) { setError(err.message); }
    else {
      setClassName(""); setClassDesc(""); setAdminCode(""); setAdminVerified(false); setAdminStep(true); setShowCreate(false);
      fetchClasses();
    }
    setLoading(false);
  };
  ...
  const joinClass = async () => {
    ...
    const { data: classData } = await supabase
      .from("classes").select("*").eq("code", joinCode.trim().toUpperCase()).single();
    if (!classData) { setError("Class not found. Check the code and try again."); setLoading(false); return; }
    const { error: err } = await supabase.from("class_members").insert({ class_id: classData.id, user_id: user.id });
    if (err) {
      if (err.message.includes("duplicate")) setError("You're already in this class!");
      else setError(err.message);
    } else { setJoinCode(""); setShowJoin(false); fetchClasses(); }
    setLoading(false);
  };
  ...
  const removeStudent = async (classId: string, studentId: string) => {
    await supabase.from("class_members").delete().eq("class_id", classId).eq("user_id", studentId);
    // Re-fetch members
  ...
  const openClass = async (cls: ClassData) => {
    ...
    const { data: memberData } = await supabase
      .from("class_members").select("user_id").eq("class_id", cls.id);

    if (memberData) {
      const studentIds = memberData.map((m) => m.user_id);
      
      // Fetch teacher profile separately
      const { data: tProfile } = await supabase
        .from("profiles").select("user_id, display_name, total_score, games_played")
        .eq("user_id", cls.teacher_id).single();
      if (tProfile) setTeacherProfile(tProfile as MemberWithProfile);
      else setTeacherProfile(null);

      // Fetch student profiles (exclude teacher)
      const studentOnlyIds = studentIds.filter(id => id !== cls.teacher_id);
      if (studentOnlyIds.length > 0) {
        const { data: profiles } = await supabase
          .from("profiles").select("user_id, display_name, total_score, games_played")
          .in("user_id", studentOnlyIds).order("total_score", { ascending: false });
  ...
  const isTeacher = selectedClass?.teacher_id === user?.id;
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col relative overflow-hidden">
      ...
      <Dialog open={showCreate} onOpenChange={(open) => {
        setShowCreate(open);
        if (!open) { setAdminCode(""); setAdminVerified(false); setAdminStep(true); setError(""); }
      }}>
          ...
          {adminStep && !adminVerified ? (
            <div className="space-y-4 pt-2">
              ...
              <div>
                ...
                <input value={adminCode} onChange={(e) => setAdminCode(e.target.value.toUpperCase())}
              ...
              <button onClick={verifyAdminCode} disabled={loading || !adminCode.trim()}
            ...
            <div className="space-y-4 pt-2">
              ...
              {error && <p className="text-destructive text-sm">{error}</p>}
              <button onClick={createClass} disabled={loading || !className.trim()}
                className="w-full py-3 bg-gradient-to-r from-accent to-primary text-accent-foreground rounded-xl font-display font-bold hover:opacity-90 transition-opacity disabled:opacity-50">
        ...
        <DialogContent className="sm:max-w-md bg-card/95 backdrop-blur-lg border-border/50">
          ...
          <div className="space-y-4 pt-2">
            <input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
              placeholder="Enter class code" maxLength={6}
            ...
            {error && <p className="text-destructive text-sm">{error}</p>}
            <button onClick={joinClass} disabled={loading || joinCode.length < 6}
            ...
            <>
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="flex items-center gap-3 p-3 bg-muted/30 border border-border/30 rounded-lg">
                  ...
                  <button onClick={() => copyCode(selectedClass.code)}
                ...
                {teacherProfile && (
                  <div>
                    <h3 className="font-display font-bold text-sm text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-2">
                      <GraduationCap className="h-4 w-4 text-accent" /> Teacher
                    </h3>
                    <div className="flex items-center gap-3 p-3 bg-accent/10 border border-accent/20 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-display font-bold text-sm text-foreground truncate">
                          {teacherProfile.display_name || "Teacher"}
                        </p>
                ...
                <div>
                  <h3 className="font-display font-bold text-sm text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                    <Trophy className="h-4 w-4 text-secondary" /> Student Leaderboard
                  </h3>
                  <div className="space-y-2">
                    {members.map((m, i) => (
                      <motion.div
                ...
                {!isTeacher && (
                  <ClassGameLauncher classId={selectedClass.id} />
                )}
